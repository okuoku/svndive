      ...
      ((= c LF)
       (cond
         ((and (= start cur) (not key))
          (values (+ start 1) #f #f))
         (key
           (let ((str (utf8->string bv value-start cur)))
            (values (+ cur 1) key str)))
         (else
           (error "Invalid MIME header sequence" start))))
      ((and (not key) (= c COLON))
       (let ((sym (string->symbol (utf8->string bv start cur))))
        (set! key sym)
               ...
               (case type
                 ...
                 ((K) 
                  ...
                  (let* ((keystart (+ next 1)) ;; Consume \n
                         ...
                         (sym (string->symbol (utf8->string bv keystart keyend))))
                 ...
                 ((V)
                  ...
                  (let* ((valuestart (+ next 1)) ;; Consume \n
                         ...
                         (str (utf8->string bv valuestart valueend)))
